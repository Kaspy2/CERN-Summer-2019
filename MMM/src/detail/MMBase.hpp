...
#include "ACTFW/Plugins/Ply/PlyWriterOptions.hpp"
#include "ACTFW/Plugins/Ply/PlyWriter.hpp"

// #include <any>

// namespace Acts {
//   using AlgorithmContext = std::any;
// }
                       ...
                       geometry_setup_t geometrySetup)
{
  ...
  FW::Options::addPlyWriterOptions(desc);
  ...
  auto tGeometry = geometry.first; // <- TrackingGeometry
  ...
  FW::Ply::PlyWriter::Config tgPlyWriterConfig = FW::Options::readPlyWriterConfig(vm, *tGeometry);
  auto tgPlyWriter = std::make_shared<FW::Ply::PlyWriter>(tgPlyWriterConfig);
  sequencer.addWriter(tgPlyWriter);
